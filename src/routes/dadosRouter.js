...
// Obtenha as credenciais do usuário autenticado (substitua pelos dados reais)
const credentials = {
...
const bucket = storage.bucket('assinatura-cliente-follow-api.appspot.com');
const file = bucket.file('seu-arquivo.pdf');
...
router.get('/download-pdf/:userId', async (req, res) => {
  try {
    ...
    // Construa a URL do arquivo no Firebase Storage
    const urlDoFirebaseStorage = `https://storage.googleapis.com/assinatura-cliente-follow-api.appspot.com/${caminhoNoFirebaseStorage}`;
